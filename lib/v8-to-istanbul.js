...
module.exports = class V8ToIstanbul {
  ...
  async load() {
    ...
    if (rawSourceMap) {
      this.path = parseSourceMapPath(rawSourceMap.sourcemap.sources[0]);
  ...
  applyCoverage(blocks) {
    blocks.forEach(block => {
      block.ranges.forEach((range, i) => {
        const { startCol, endCol } = this._maybeRemapStartColEndCol(range);
  ...
  _maybeRemapStartColEndCol(range) {
    ...
    let endCol = Math.min(
      this.source.eof,
      range.endOffset - this.source.wrapperLength
    );
...
function parseSourceMapPath(scriptPath) {
  return `./.nyc_output/js/${scriptPath
    .replace('webpack:///', 'webpack:/')
    .replace('./', '')}`;
}
